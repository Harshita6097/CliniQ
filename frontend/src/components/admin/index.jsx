    ...
    <>
      <div className="bg-white rounded-lg border border-stone shadow-soft p-5 hover:shadow-pop transition-all duration-200">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="font-mono text-xs text-admin-dark mt-0.5">{d.specialization}</p>
            <p className="text-xs text-ink-soft mt-0.5">{d.userId?.email}</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-2 mb-4">
          {d.consultationFee > 0 && (
            <span className="font-mono text-[11px] bg-ok-tint text-[#3a5c38] border border-ok/20 px-2.5 py-0.5 rounded-full">₹{d.consultationFee}</span>
          )}
          ...
          {d.leaveDays?.length > 0 && (
            <span className="font-mono text-[11px] bg-warn-tint text-doctor-dark border border-warn/20 px-2.5 py-0.5 rounded-full">{d.leaveDays.length} leave day{d.leaveDays.length !== 1 ? 's' : ''}</span>
          )}
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => onEdit(d)} className="flex-1 text-xs font-semibold text-admin-dark bg-admin-tint hover:bg-admin-tint2 border border-admin/20 px-3 py-2 rounded-md transition-colors">Edit</button>
          <button onClick={() => onLeave(d)} className="flex-1 text-xs font-semibold text-doctor-dark bg-warn-tint hover:bg-warn-tint2 border border-warn/20 px-3 py-2 rounded-md transition-colors">Mark Leave</button>
          {isActive ? (
      ...
      <Modal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} title="Deactivate doctor?">
        <div className="mb-5 flex gap-3 items-start bg-danger-tint border border-danger/30 rounded-md px-4 py-3">
          ...
          <p className="text-sm text-[#7a2e29]">Dr. {d.userId?.name} will no longer appear in patient searches and cannot log in.</p>
        </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button type="button" onClick={onAdd} className="text-xs font-semibold text-admin-dark bg-admin-tint hover:bg-admin-tint2 px-3 py-1.5 rounded-md transition-colors">+ Add row</button>
      </div>
      {duplicates.length > 0 && (
