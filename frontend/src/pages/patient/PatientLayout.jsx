  ...
  const SidebarContent = () => (
    <>
      <div className="px-6 py-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-patient-tint font-medium">Patient Portal</p>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="mx-4 mb-4 rounded-md bg-white/10 border border-white/20 px-4 py-3 text-left hover:bg-white/20 transition-colors w-[calc(100%-2rem)] group"
        title="Change password"
      >
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-patient-light/40 flex items-center justify-center text-white text-sm font-bold shrink-0">
            {initials}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-white truncate">{user.name}</p>
            <p className="text-xs text-patient-tint flex items-center gap-1">
              Change password
      ...
      <nav className="flex-1 px-3 space-y-0.5">
        {links.map(({ to, label, end, icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `relative flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium transition-all duration-150 overflow-hidden ${
                isActive ? 'bg-white/15 text-white' : 'text-patient-tint hover:bg-white/10 hover:text-white'
              }`
            }
          >
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute left-0 top-2 bottom-2 w-[3px] bg-white rounded-r-full" />}
                {icon}
                {label}
              </>
            )}
      ...
      <div className="px-3 py-4 mt-4 border-t border-white/10">
        <button
          ...
          className="flex items-center gap-3 w-full rounded-md px-3 py-2.5 text-sm font-medium text-patient-tint hover:bg-white/10 hover:text-danger-tint transition-all duration-150"
        >
    ...
    <div className="flex min-h-screen bg-paper">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-stone">
          <button onClick={() => setMobileOpen(true)} className="p-2 rounded-md hover:bg-paper-dim focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-patient" aria-label="Open menu">
          ...
          <div className="flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-patient"><path d="M2 12h4l2-7 4 14 3-9 2 2h5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
      ...
      {showChangePw && <ChangePasswordModal accentColor="teal" onClose={() => setShowChangePw(false)} />}
    </div>
  );
}
